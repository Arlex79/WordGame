...
class Server:
        ...
        if msg_.type == 'set nick':
            ...
            try:
                if self.check_nick(new_nick):
                    ...
                    self.send(connection, Msg(f'You nick set to {new_nick}!', None, 'warning'))
    ...
    def send_new_word(self):
        ...
        m = Msg(f'\\"{new_word}\\" and letter set to: {self.letter_now}',
        ...
        if self.is_word_startswith_letter_now(word):
            if self.is_not_already_spoken(word):
                if self.is_valid_word(word):
                    ...
                    m = Msg(f'Word is not valid!', 'server', 'word game data')
                    self.send(conn, m)
                    return


            else:
                m = Msg('Word has already been spoken', 'server', 'word game data')
                self.send(conn, m)

        else:
            m = Msg(f'Word not startswith {self.letter_now}', 'server', 'word game data')
            self.send(conn, m)
            return
